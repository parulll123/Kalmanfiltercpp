...
const int jml_dt = 24;
int dt_pln[jml_dt];
int prev_dt_pln[jml_dt];  // Store previous values for comparison
bool dataChanged = false; // Flag to indicate if data has changed

// Define which array indexes are important for change detection
// These are the RST phases we're monitoring (based on your mappings)
const int importantIndexes[] = {12, 13, 14, 15, 16, 17, 18, 19, 20};
const int numImportantIndexes = sizeof(importantIndexes) / sizeof(importantIndexes[0]);

unsigned long minPublishInterval = 100; // Minimum time between publishes even with changes (prevent flooding)
bool forcePublish = false;              // Flag for forced publish based on timer
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (String(topic) == "event/interval")
  {
    ...
    if (interval < 100)
      interval = 100; // Minimum 100ms to prevent flooding
  ...
  // Handle force publish command
  if (String(topic) == "ESP32/command" && message == "publish")
  {
    forcePublish = true;
    Serial.println("Force publish requested");
  }
}
...
void setup()
{
  ...
  // Initialize previous values to an impossible state to force first publish
...
// Function to check if data has changed in important indexes
bool checkDataChanged()
{
  for (int i = 0; i < numImportantIndexes; i++)
  {
    int idx = importantIndexes[i];
    if (dt_pln[idx] != prev_dt_pln[idx])
    {
      // Store which values changed for debug
      Serial.print("Change detected at index ");
      Serial.print(idx);
      Serial.print(": ");
      Serial.print(prev_dt_pln[idx]);
      Serial.print(" -> ");
      Serial.println(dt_pln[idx]);
      return true;
    }
  }
  return false;
...
// Function to update previous values after publishing
void updatePreviousValues()
{
  for (int i = 0; i < jml_dt; i++)
  {
    prev_dt_pln[i] = dt_pln[i];
  }
}
...
void publishData()
{
  ...
  String payload = "{\"Livebit\":" + String(liveBitsCount) + ",\"R1\":" + String(dt_pln[12]) + ",\"S1\":" + String(dt_pln[13]) + ",\"T1\":" + String(dt_pln[14]) +
                   ",\"R2\":" + String(dt_pln[15]) + ",\"S2\":" + String(dt_pln[16]) + ",\"T2\":" + String(dt_pln[17]) +
  ...
  if (mqttClient.connected())
  {
    if (mqttClient.publish(mqtt_topic, payload.c_str()))
    {
      ...
      // Update previous values after successful publish
...
void loop()
{
  ...
  if (cur_time_get - old_time_get >= 3000)
  {
    ...
    dt_pln[16] = buf_dt3[7]; // S2
    dt_pln[17] = buf_dt3[6]; // T2
    ...
    // Check if data has changed in important indexes
    if (!dataChanged)
    {
      dataChanged = checkDataChanged();
      if (dataChanged)
      {
        Serial.println("Data change detected, will publish soon");
      }
    }

    /*
     Also check if we need a forced publish based on timer
    This ensures we publish at least every 'interval' milliseconds
    even if no changes are detected*/
    if (currenttimePublish - lastPublishTime >= interval)
    {
      forcePublish = true;
    }

    // Publish if data changed AND minimum interval elapsed, OR if forced publish
    if ((dataChanged && currenttimePublish - lastPublishTime >= minPublishInterval) || forcePublish)
    {
      if (forcePublish && !dataChanged)
      {
        Serial.println("Publishing due to timer (no changes detected)");
      }
      else
      {
        Serial.println("Publishing due to detected changes");
      }
...
void reconnect()
{
  ...
  { // Limit connection attempts to prevent blocking
    ...
    if (mqttClient.connect(clientId.c_str()))
    {
      ...
      // Subscribe to command topic to force publish
      mqttClient.subscribe("ESP32/command");
      Serial.println("Subscribed to ESP32/command topic");
    }
    else
    {
